...
-->
					<#list  list as item >
						<tr class="text-c">
							...
							<td class="f-14 td-manage"><#--${ctx}/companyCulture/saveOrUpdate-->
                                <a style="text-decoration:none" class="ml-5" onClick="article_edit('编辑','${ctx}/companyCulture/content?id='+'${item.id}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
...
function article_add(){
	var url='${ctx}/companyCulture/addPage';
	var index = layer.open({
		type: 2,
		area: ['90%', '90%'],
		// offset: 'rb', //右下角弹出
...
function article_del(obj,number){
	layer.confirm('确认要删除吗？',function(index){
	   ...
	   // alert(id);
		$.ajax({
            type: 'POST',
			url: '${ctx}/companyCulture/delete/'+number,
