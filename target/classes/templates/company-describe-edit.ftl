...
<article class="page-container">
   ...
   <#--companydescribe-->
    ...
    <input    type="hidden"  value="${companydescribe.id}"  id="id" >

    <form id="fileForm"  class="form form-horizontal" enctype="multipart/form-data">
    <div class="row cl">
        <label class="form-label col-xs-4 col-sm-2">图片上传：</label>
        <div class="formControls col-xs-8 col-sm-9">
          <#--  <#if companydescribe.url !="">
                <img   alt="图片"  style="opacity: 1"  width=90px height=90px id="pic" src="${companydescribe.url}" >
                <a style="text-decoration:none" class="ml-5"  id="delImg" onClick="img_del('${companydescribe.url}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
            </#if>-->
            <a id="apic" >
                <a id="remark1"></a>
            </a>
...
<script type="text/javascript">
	...
	//	var  content = UE.getEditor('editor').getPlainTxt();
		...
		$.ajax({
            type: 'POST',
            url:"${ctx}/companyDescribe/saveOrUpdate",
