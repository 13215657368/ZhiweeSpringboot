...
<div class="page-container">
    ...
    <form class="form form-horizontal" id="form-article-add2">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>刊物名称：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text"  value="${pub.title}" id="title" name="title">
            </div>
        </div>
    </form>
    ...
    <form id="fileForm2"  class="form form-horizontal" enctype="multipart/form-data">
        ...
        <div class="row cl">
           &nbsp;&nbsp; <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>刊物封面：</label>
           <#-- <input id="file-product-category" class="file" name="uploadFile" type="file"  data-min-file-count="1" />
            <input type="button" id="pub" onclick="publicationImg()" value="上传">-->
            ...
            <button  id="pub"  onclick="publicationImg()"  class="btn btn-primary radius upload-btn" type="button" style="width: 130px;height: 30px; "><i class="Hui-iconfont">&#xe642;</i>上传</button>




            <input  type="hidden" id="puburl"  value="<#if pub.img!="">${pub.img}</#if>" >
            &nbsp;&nbsp; &nbsp;&nbsp;
        <#if pub.img!="">
            <img  alt='图片缩略图' id='showIs2'  src="${pub.img}" style='opacity: 1'  width=90px height=90px  /> <a style='text-decoration:none' class='ml-5'  id='delImg4' onClick='img_del()' title='删除'>
            <a style="text-decoration:none" class="ml-5"  id="delImg5" onClick="img_del()" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
        </#if>
            <a id="info2"></a>
            <a id="remark2"></a>
        </div>
    ...
    <form  class="form form-horizontal" id="form-article-add">
        ...
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>版页：</label>
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select name="pagenum" id="pagenum" class="select">
					<option value="0">请选择版页数</option>
        ...
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>刊物内容：</label>
            <input  type="hidden" id="imgUrl">
            <div class="formControls col-xs-8 col-sm-9">
                <div class="uploader-list-container">
                    ...
                    <div class="statusBar" style="display:none;">
                        ...
                        <div class="btns">
                            ...
                            &nbsp;
                        <#-- <button    style=" background:#f0f0f0; height:39px;width:100px" onclick="imgInsert()">图片入库</button>-->
        ...
        <div  id="maps" class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>缩略图：</label>
            <div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
                <#if pictureList ??>
                    <#list   pictureList as item>
                        <a id="pageNum2${item.id}" >第${item.pagenum}页</a>
                        <img  alt="图片"  id="showIs2${item.id}"  style="opacity: 1" src="${item.url}" width=90px height=90px id="pic" />
                        <a style="text-decoration:none" class="ml-5"  id="delImg2${item.id}" onClick="img_del3('${item.id}','${item.url}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
                         &nbsp;&nbsp;&nbsp;
                        <input  type="hidden"  value="${item.url}"  id="titleImgUrl">
                    </#list>
                </#if>

                <a id="remark1" style="color: deepskyblue">
    ...
    <div class="row cl">
        <div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2" align="center">
            ...
            <button onClick="publication_save_submit();"    class="btn btn-primary radius" type="button"><i class="Hui-iconfont">&#xe632;</i>发布</button>
...
<script type="text/javascript">
    ...
    function imgInsert(pagenum) {
      ...
      /*  var pageNum = $("#pagenum option:selected").val();
        ...
        $.ajax({
            type: 'POST',
            url:"${ctx}/publication/pubContent/save",
            data:{
                pnumber: ${pub.number},
                url:url,
                pagenum: pagenum
        ...
        $(function() {
            ...
            uploader.on('uploadSuccess', function(file,response) {
              ...
              //  console.log(response.fileUrl);
                ...
                $("#imgUrl").val(response.fileUrl);
                $("#remark1").append("<img  alt='图片缩略图'  id='id"+i+"' src='"+response.fileUrl+"'    style='opacity: 1'  width=90px height=90px /> ");
                $('#remark1').append('<a style="text-decoration:none" id="del'+i+'" class="ml-5"   onClick="img_del4(\''+i+'\',\''+ response.fileUrl + '\')"  href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>');
                i+=1;
                $("#imgId").val(i);
    ...
    function img_del() {
        var  pathName= $("#puburl").val();

        $.ajax({
            type: 'POST',
            url:"${ctx}/publication/titleImg/delete",
            data:{
                id:${pub.id},
                img:pathName,
                number:${pub.number}
        ...
        $.ajax({
            ...
            success:function (result) {
                ...
                if(result.fileUrl!=null){
                    $("#puburl").val(result.fileUrl);
                    /*     alert("图片上传成功，请点击图片入库");*/
                    // $('#showImg').attr('src',result.fileUrl);
                    $("#remark2").html("");
                    $('#info2').html("当前进度:100%");
                    $("#remark2").append("<img  alt='图片缩略图' id='showIs2'  style='opacity: 1'  width=90px height=90px  /> <a style='text-decoration:none' class='ml-5'  id='delImg' onClick='img_del()' title='删除'>")
                    $('#showIs2').attr('src',result.fileUrl);
                    //  $('#remark2').append('<button type="button"   onclick="img_del(2)">删除</button>');
    ...
    function publication_save_submit() {
        var title = $("#title").val();
        var  img = $("#puburl").val();
